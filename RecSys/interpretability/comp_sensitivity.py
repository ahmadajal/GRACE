"""
This scripts computes the COMP metric of the sensitivity analysis method.
"""
import os
import sys
import torch
...
def sensitivity_analysis_top(graph: RecGraphData):
    deg = torch_sparse.sum(graph.edge_index, 1)[graph.x[:, 0]]
    deg = 1 / (1e-12 + torch.sqrt(deg))
    return torch.argsort(deg[:graph.num_users]), torch.argsort(deg[graph.num_users:])


def get_subgraph_and_all(u: int, i: int, graph):
    ...
    top_users, top_items = torch.argsort(-deg), torch.argsort(-deg)
...
if __name__ == "__main__":
    ...
    for u in tqdm(range(NB_USER)):
        for i in tqdm(np.argsort(-s[u])[:NB_ITEM]):
            comp_at_ui = []
            subgraph, idx_u, idx_i, adj_mat, top_users, top_items = get_subgraph_and_all(u, i+train_graph.num_users, train_graph)
    ...
    np.save(f"scores/comp_sensitivity_{NB_USER}users{NB_ITEM}items.npy", comp)
