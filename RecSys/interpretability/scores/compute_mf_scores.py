""" Compute MF scores for all users and items in the test set.
...
with torch.no_grad():
    ...
    scores = model.get_all_scores(train_graph).cpu().numpy()
    scores = scores.astype("float16")

# Save scores
np.save(os.path.join("RecSys", "interpretability", "scores", "mf_scores.npy"), scores)
